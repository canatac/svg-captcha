...
const createCaptcha = async (text, options) => {
...
*/
const create = async (width, height, options) => {
	const userOption = await getOption(width, height, options);
	const text = await random.captchaText(userOption);
	const data = await createCaptcha(text, userOption);
